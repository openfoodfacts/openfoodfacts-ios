...
class ProductDetailViewControllerTests: XCTestCase {
    ...
    // MARK: - viewDidLoad
    func skiptestAfterViewDidLoadButtonBarPagerTabStripIsSetup() {
...
//        expect(rows[4].value as? [String]).to(equal(product.brands))
        ...
        expect(rows[6].label).to(equal(InfoRowKey.manufacturingPlaces.localizedString))
        expect(rows[6].value as? String).to(equal(product.manufacturingPlaces))
        expect(rows[6].cellType == InfoRowTableViewCell.self).to(beTrue())
        expect(rows[7].label).to(equal(InfoRowKey.origins.localizedString))
        expect(rows[7].value as? String).to(equal(product.origins))
        expect(rows[7].cellType == InfoRowTableViewCell.self).to(beTrue())
        expect(rows[8].label).to(equal(InfoRowKey.categories.localizedString))
//        expect(rows[8].value as? [String]).to(equal(product.categories))
        expect(rows[8].cellType == InfoRowTableViewCell.self).to(beTrue())
        expect(rows[9].label).to(equal(InfoRowKey.labels.localizedString))
        expect(rows[9].value as? [String]).to(equal(product.labels))
        expect(rows[9].cellType == InfoRowTableViewCell.self).to(beTrue())
        expect(rows[10].label).to(equal(InfoRowKey.citiesTags.localizedString))
        expect(rows[10].value as? [String]).to(equal(product.citiesTags))
        expect(rows[10].cellType == InfoRowTableViewCell.self).to(beTrue())
        expect(rows[11].label).to(equal(InfoRowKey.stores.localizedString))
        expect(rows[11].value as? [String]).to(equal(product.stores))
        expect(rows[11].cellType == InfoRowTableViewCell.self).to(beTrue())
        expect(rows[12].label).to(equal(InfoRowKey.countries.localizedString))
        expect((rows[12].value as? [NSAttributedString])?.map { $0.string }).to(equal(product.countriesTags))
        expect(rows[12].cellType == InfoRowTableViewCell.self).to(beTrue())
...
//        expect(rows[6].cellType == InfoRowTableViewCell.self).to(beTrue())
        ...
        expect((rows[0].value as? NutritionScoreTableRow)?.nutriscore).to(equal(product.nutriscore))
